use revm::{
    ...
    primitives::{EthChainSpec, ResultAndState},
...
fn run_transaction<EXT, DB: DatabaseRefDebugError>(
    db: DB,
    ext: EXT,
    register_handles_fn: HandleRegister<EthChainSpec, EXT, WrapDatabaseRef<DB>>,
) -> anyhow::Result<(ResultAndState<EthChainSpec>, DB)> {
