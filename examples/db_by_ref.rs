use revm::{
    ...
    primitives::{MainnetChainSpec, ResultAndState},
...
fn run_transaction<EXT, DB: DatabaseRefDebugError>(
    db: DB,
    ext: EXT,
    register_handles_fn: HandleRegister<MainnetChainSpec, EXT, WrapDatabaseRef<DB>>,
) -> anyhow::Result<(ResultAndState<MainnetChainSpec>, DB)> {
