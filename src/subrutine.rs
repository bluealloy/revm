...
pub struct SubRutine {
    ...
    depth: usize,
    /// is static subrutine. SSTORE and various CALLs are not permited
    is_static: bool,
...
impl SubRutine {
    ...
    ///
    pub fn checkpoint_commit(&mut self, checkpoint: SubRutineCheckpoint) -> Result<(), ExitError> {
        self.depth -= 1;
        // we are continuing to use present checkpoint because it is merge between ours and parents
        Ok(())
    }

    pub fn checkpoint_revert(&mut self, checkpoint: SubRutineCheckpoint) {
    ...
    pub fn checkpoint_discard(&mut self, checkpoint: SubRutineCheckpoint) {
        self.logs.truncate(checkpoint.log_i);
        // TODO revert all changes from changeset
        self.changeset.truncate(checkpoint.changeset_i);
        self.depth -= 1;
    ...
    pub fn destroy_storage(&mut self, address: H160) {
        let acc = self.state.get_mut(&address).unwrap();
        // if there is no account in this subrutine changeset,
        // save it so that it can be restored if subrutine needs to be rediscarded.
        self.changeset
            .last_mut()
            .unwrap()
            .entry(address.clone())
