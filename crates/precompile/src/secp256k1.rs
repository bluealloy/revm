...
use revm_primitives::{alloy_primitives::B512, Bytes, PrecompileErrors, B256};
...
pub fn ec_recover_run(input: &Bytes, gas_limit: u64) -> PrecompileResult {
    ...
    if !(input[32..63].iter().all(|&b| b == 0) && matches!(input[63], 27 | 28)) {
        return PrecompileResult::ok(ECRECOVER_BASE, Bytes::new());
    }
    ...
    let out = secp256k1::ecrecover(sig, recid, msg)
        .map(|o| o.to_vec().into())
        .unwrap_or_default();
    PrecompileResult::ok(ECRECOVER_BASE, out)
}
