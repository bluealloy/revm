...
fn parse_g1_compressed(bytes: &[u8; 48]) -> Result<G1Affine, PrecompileHaltReason> {
    G1Affine::deserialize_compressed(&bytes[..]).map_err(|_| PrecompileHaltReason::KzgInvalidG1Point)
}

/// Read a scalar field element from bytes and verify it's canonical
fn read_scalar_canonical(bytes: &[u8; 32]) -> Result<Fr, PrecompileHaltReason> {
