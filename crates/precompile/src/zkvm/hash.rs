...
extern "C" {
    ...
    /// zkVM implementation of RIPEMD-160 hash function.
    ///
    /// # Arguments
    /// * `input_ptr` - Pointer to input data
    /// * `input_len` - Length of input data
    /// * `output_ptr` - Pointer to 20-byte output buffer
    ///
    /// # Returns
    /// * 0 on success
    /// * Non-zero on error
    fn zkvm_ripemd160_impl(input_ptr: *const u8, input_len: u32, output_ptr: *mut u8) -> i32;
}
...
/// Compute RIPEMD-160 hash using zkVM implementation.
pub fn ripemd160_hash(input: &[u8]) -> [u8; 20] {
    let mut output = [0u8; 20];

    let result =
        unsafe { zkvm_ripemd160_impl(input.as_ptr(), input.len() as u32, output.as_mut_ptr()) };
