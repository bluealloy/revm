...
mod arkworks_general {
    use super::{Fq, StdRng, FP_LENGTH, FP_PAD_BY, PADDED_FP_LENGTH};
    ...
    pub(super) fn random_points<P: AffineRepr>(num_points: usize, rng: &mut StdRng) -> Vec<P> {
        (0..num_points).map(|_| P::rand(rng)).collect()
    }

    pub(super) fn random_field<F: Field>(num_scalars: usize, rng: &mut StdRng) -> Vec<F> {
        (0..num_scalars).map(|_| F::rand(rng)).collect()
    }
...
fn g1_msm_test_vectors(msm_size: usize, rng: &mut StdRng) -> PrecompileInput {
    let points: Vec<G1Affine> = arkworks_general::random_points(msm_size, rng);
    let scalars: Vec<Fr> = arkworks_general::random_field(msm_size, rng);

    let mut input = Vec::new();
    for (point, scalar) in points.iter().zip(scalars.iter()) {
        input.extend(encode_bls12381_g1_point(point));
        input.extend(arkworks_general::encode_field_32_bytes(scalar));
    }
...
fn g2_msm_test_vectors(msm_size: usize, rng: &mut StdRng) -> PrecompileInput {
    let points: Vec<G2Affine> = arkworks_general::random_points(msm_size, rng);
    let scalars: Vec<Fr> = arkworks_general::random_field(msm_size, rng);
...
/// Add benches for the BLS12-381 G1 msm precompile over random inputs, sized to cover the points
/// where blst switches multiplication algorithm.
pub fn add_g1_msm_benches(group: &mut BenchmarkGroup<'_, WallTime>) {
...
/// Add benches for the BLS12-381 G2 msm precompile over random inputs, sized to cover the points
/// where blst switches multiplication algorithm.
pub fn add_g2_msm_benches(group: &mut BenchmarkGroup<'_, WallTime>) {
