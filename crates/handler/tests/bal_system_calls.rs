...
#[test]
fn separately_committed_system_calls_restore_the_index_start() {
    for spec in [SpecId::PRAGUE, SpecId::AMSTERDAM] {
        let mut evm = evm(spec);
        system_call_and_commit(&mut evm, Some(42), 0);
        system_call_and_commit(&mut evm, Some(0), 42);
        system_call_and_commit(&mut evm, None, 0);
...
#[test]
fn system_calls_restore_the_previous_index_value_without_losing_history() {
    for spec in [SpecId::PRAGUE, SpecId::AMSTERDAM] {
        let mut evm = evm(spec);
        system_call_and_commit(&mut evm, Some(7), 0);
        evm.db_mut().bump_bal_index();
        system_call_and_commit(&mut evm, Some(42), 7);
        system_call_and_commit(&mut evm, Some(7), 42);
        ...
        assert_eq!(slot.changes[0].new_value, U256::from(7));
    }
}
