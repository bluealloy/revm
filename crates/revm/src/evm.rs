use crate::{
    ...
    CallStackFrame, EvmContext, FrameOrResult, Inspector,
};
...
pub struct Evm<'a, SPEC: Spec, EXT, DB: Database> {
    pub context: EvmContext<'a, EXT, DB>,
    pub inspector: Option<&'a mut dyn Inspector<EXT, DB>>,
...
impl<'a, SPEC: Spec + 'static, EXT, DB: Database> Evm<'a, SPEC, EXT, DB> {
    pub fn new_with_spec(
        db: &'a mut DB,
        env: &'a mut Env,
        external: EXT,
        inspector: Option<&'a mut dyn Inspector<EXT, DB>>,
...
impl<'a, SPEC: Spec + 'static, EXT, DB: Database> Transact<DB::Error> for Evm<'a, SPEC, EXT, DB> {
    #[inline]
...
pub fn new_evm<'a, EXT, DB: Database>(
    env: &'a mut Env,
    db: &'a mut DB,
    external: EXT,
    insp: Option<&'a mut dyn Inspector<EXT, DB>>,
