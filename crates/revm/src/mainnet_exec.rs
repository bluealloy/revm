...
where
    CTX: ContextSetters + ContextT<Journal: Journal<FinalOutput = (EvmState, Vec<Log>)>>,
...
where
    CTX: ContextSetters
        + ContextT<Journal: Journal<FinalOutput = (EvmState, Vec<Log>)>, Db: DatabaseCommit>,
    PRECOMPILES: PrecompileProvider<Context = CTX, Output = InterpreterResult>,
{
