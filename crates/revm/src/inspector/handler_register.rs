use crate::{
    db::Database,
    handler::{
        execution::LastFrameReturnTrait, register::EvmHandler, FrameCallTrait, FrameCreateTrait,
        InsertCallOutcomeTrait, InsertCreateOutcomeTrait,
    },
    interpreter::{
    ...
    primitives::EVMError,
    Context, Evm, EvmContext, Frame, FrameOrResult, FrameResult, Inspector, JournalEntry,
};
use core::cell::RefCell;
use revm_interpreter::{
    opcode::InstructionTables, CallInputs, CallOutcome, CreateInputs, CreateOutcome, SharedMemory,
};
...
pub fn inspector_handle_register<'a, DB: Database + 'a, EXT: GetInspector<DB> + 'a>(
    ...
    struct InspectorHandler<'a, DB: Database + 'a, EXT: GetInspector<DB> + 'a> {
        call_input_stack: Rc<RefCell<Vec<Box<CallInputs>>>>,
        create_input_stack: Rc<RefCell<Vec<Box<CreateInputs>>>>,
        base_create: Box<dyn FrameCreateTrait<EXT, DB> + 'a>,
        base_call: Box<dyn FrameCallTrait<EXT, DB> + 'a>,
        base_insert_call_outcome: Box<dyn InsertCallOutcomeTrait<EXT, DB> + 'a>,
        base_insert_create_outcome: Box<dyn InsertCreateOutcomeTrait<EXT, DB> + 'a>,
        base_last_frame_return: Box<dyn LastFrameReturnTrait<EXT, DB> + 'a>,
    }

    impl<DB: Database, EXT: GetInspector<DB>> Clone for InspectorHandler<'_, DB, EXT> {
        fn clone(&self) -> Self {
            Self {
                call_input_stack: self.call_input_stack.clone(),
                create_input_stack: self.create_input_stack.clone(),
                base_create: self.base_create.clone(),
                base_call: self.base_call.clone(),
                base_insert_call_outcome: self.base_insert_call_outcome.clone(),
                base_insert_create_outcome: self.base_insert_create_outcome.clone(),
                base_last_frame_return: self.base_last_frame_return.clone(),
            }
        }
    }

    #[derive(Clone)]
    struct InsectorFrameCreate<
        DB: Database,
        EXT: GetInspector<DB>,
        T: FrameCreateTrait<EXT, DB> + Clone,
    > {
        base_create: T,
        phantom: std::marker::PhantomData<(EXT, DB)>,
    }

    impl<DB: Database, EXT: GetInspector<DB>, T: FrameCreateTrait<EXT, DB> + Clone>
        FrameCreateTrait<EXT, DB> for InsectorFrameCreate<DB, EXT, T>
    {
        #[inline]
        fn create(
            &mut self,
            ...
            if let Some(outcome) = inspector.create(&mut ctx.evm, &mut inputs) {
                self.create_input_stack.borrow_mut().push(inputs.clone());
                return Ok(FrameOrResult::Result(FrameResult::Create(outcome)));
            }
            self.create_input_stack.borrow_mut().push(inputs.clone());

            let mut frame_or_result = self.base_create.create(ctx, inputs);
    ...
    impl<DB: Database, EXT: GetInspector<DB>> FrameCreateTrait<EXT, DB>
        for InspectorHandler<'_, DB, EXT>
    {
        #[inline]
        fn create(
            &mut self,
    ...
    impl<DB: Database, EXT: GetInspector<DB>> FrameCallTrait<EXT, DB>
        for InspectorHandler<'_, DB, EXT>
    {
        #[inline]
        fn call(
            ...
            self.call_input_stack.borrow_mut().push(inputs.clone());
            if let Some(outcome) = outcome {
                return Ok(FrameOrResult::Result(FrameResult::Call(outcome)));
            }

            let mut frame_or_result = self.base_call.call(ctx, inputs);
            if let Ok(FrameOrResult::Frame(frame)) = &mut frame_or_result {
                ctx.external
                    .get_inspector()
                    .initialize_interp(frame.interpreter_mut(), &mut ctx.evm)
    ...
    impl<DB: Database, EXT: GetInspector<DB>> InsertCallOutcomeTrait<EXT, DB>
        for InspectorHandler<'_, DB, EXT>
    {
        #[inline]
        fn insert_call_outcome(
            ...
            let call_inputs = self.call_input_stack.borrow_mut().pop().unwrap();
            outcome = ctx
                .external
                .get_inspector()
                .call_end(&mut ctx.evm, &call_inputs, outcome);
            self.base_insert_call_outcome
                .insert_call_outcome(ctx, frame, shared_memory, outcome)
        }
    }

    impl<DB: Database, EXT: GetInspector<DB>> InsertCreateOutcomeTrait<EXT, DB>
        for InspectorHandler<'_, DB, EXT>
    {
        #[inline]
        fn insert_create_outcome(
            ...
            let create_inputs = self.create_input_stack.borrow_mut().pop().unwrap();
            outcome =
                ctx.external
                    .get_inspector()
                    .create_end(&mut ctx.evm, &create_inputs, outcome);
            self.base_insert_create_outcome
                .insert_create_outcome(ctx, frame, outcome)
        }
    }

    impl<DB: Database, EXT: GetInspector<DB>> LastFrameReturnTrait<EXT, DB>
        for InspectorHandler<'_, DB, EXT>
    {
        #[inline]
        fn last_frame_return(
            ...
            self.base_last_frame_return
                .last_frame_return(ctx, frame_result)
        }
    }

    let inspector_handler = InspectorHandler::<'a, DB, EXT> {
        call_input_stack: call_input_stack.clone(),
        create_input_stack: create_input_stack.clone(),
        base_create: handler.execution.create.clone(),
        base_call: handler.execution.call.clone(),
        base_insert_call_outcome: handler.execution.insert_call_outcome.clone(),
        base_insert_create_outcome: handler.execution.insert_create_outcome.clone(),
        base_last_frame_return: handler.execution.last_frame_return.clone(),
    };

    handler.execution.create = Box::new(inspector_handler.clone());
    handler.execution.call = Box::new(inspector_handler.clone());
    handler.execution.insert_call_outcome = Box::new(inspector_handler.clone());
    handler.execution.insert_create_outcome = Box::new(inspector_handler.clone());
    handler.execution.last_frame_return = Box::new(inspector_handler);
}
