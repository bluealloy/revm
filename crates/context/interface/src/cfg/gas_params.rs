...
impl GasParams {
    ...
    fn eip2780_base_to_value_gas(&self, is_create: bool, info: &Eip2780TxInfo) -> u64 {
        let to_is_self = info.to == Some(info.sender);
        ...
        // tx.value charge
        let value_cost = if info.value.is_zero() || to_is_self {
            0
        } else if is_create {
...
pub struct Eip2780TxInfo {
    /// Transaction sender.
    pub sender: Address,
    /// Transaction recipient. `None` for `TxKind::Create`.
