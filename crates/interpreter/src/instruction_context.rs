...
pub struct InstructionContext<'a, EXT: Sized, H: ?Sized, ITy: InterpreterTypes<Extend = EXT>> {
    /// Reference to the interpreter containing execution state (stack, memory, gas, etc).
    pub interpreter: &'a mut Interpreter<EXT, ITy>,
    /// Reference to the host interface for accessing external blockchain state.
    pub host: &'a mut H,
}

impl<EXT: Sized, H: ?Sized, ITy: InterpreterTypes<Extend = EXT>> std::fmt::Debug for InstructionContext<'_, EXT, H, ITy> {
