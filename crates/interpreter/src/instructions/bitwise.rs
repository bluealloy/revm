...
mod tests {
    ...
    use revm_primitives::{uint, Env, EthereumWiring, LatestSpec, U256};
    ...
    fn test_shift_left() {
        ...
        for test in test_cases {
            ...
            shl::<DummyHost<EthereumWiring>, LatestSpec>(&mut interpreter, &mut host);
            pop!(interpreter, res);
    ...
    fn test_logical_shift_right() {
        ...
        for test in test_cases {
            ...
            push!(interpreter, test.value);
            push!(interpreter, test.shift);
            shr::<DummyHost<EthereumWiring>, LatestSpec>(&mut interpreter, &mut host);
            pop!(interpreter, res);
            assert_eq!(res, test.expected);
        }
    }
    ...
    fn test_arithmetic_shift_right() {
        ...
        for test in test_cases {
            ...
            push!(interpreter, test.value);
            push!(interpreter, test.shift);
            sar::<DummyHost<EthereumWiring>, LatestSpec>(&mut interpreter, &mut host);
            pop!(interpreter, res);
            assert_eq!(res, test.expected);
        }
    }
    ...
    fn test_byte() {
        ...
        let mut host = DummyHost::new(Env::<EthereumWiring>::default());
        let mut interpreter = Interpreter::new(Contract::default(), u64::MAX, false);
