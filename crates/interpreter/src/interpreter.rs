...
use crate::{
    ...
    instructions::{
        arithmetic, bitwise, block_info, contract, control, gas, memory, system, tx_info,
    },
    interpreter_types::*,
...
// Aliased imports to avoid naming conflicts with local modules and parameters
use crate::instructions::host as host_instructions;
use crate::instructions::stack as stack_instructions;
...
fn execute_instruction<IW: InterpreterTypes, H: Host + ?Sized>(
    ...
    match opcode {
        ...
        BALANCE => host_instructions::balance(InstructionContext { interpreter, host }),
        ...
        EXTCODESIZE => host_instructions::extcodesize(InstructionContext { interpreter, host }),
        EXTCODECOPY => host_instructions::extcodecopy(InstructionContext { interpreter, host }),
        ...
        EXTCODEHASH => host_instructions::extcodehash(InstructionContext { interpreter, host }),
        BLOCKHASH => host_instructions::blockhash(InstructionContext { interpreter, host }),
        ...
        SELFBALANCE => host_instructions::selfbalance(InstructionContext { interpreter, host }),
        ...
        POP => stack_instructions::pop(InstructionContext { interpreter, host }),
        ...
        SLOAD => host_instructions::sload(InstructionContext { interpreter, host }),
        SSTORE => host_instructions::sstore(InstructionContext { interpreter, host }),
        ...
        TLOAD => host_instructions::tload(InstructionContext { interpreter, host }),
        TSTORE => host_instructions::tstore(InstructionContext { interpreter, host }),
        MCOPY => memory::mcopy(InstructionContext { interpreter, host }),

        PUSH0 => stack_instructions::push0(InstructionContext { interpreter, host }),
        PUSH1 => stack_instructions::push::<1, _, _>(InstructionContext { interpreter, host }),
        PUSH2 => stack_instructions::push::<2, _, _>(InstructionContext { interpreter, host }),
        PUSH3 => stack_instructions::push::<3, _, _>(InstructionContext { interpreter, host }),
        PUSH4 => stack_instructions::push::<4, _, _>(InstructionContext { interpreter, host }),
        PUSH5 => stack_instructions::push::<5, _, _>(InstructionContext { interpreter, host }),
        PUSH6 => stack_instructions::push::<6, _, _>(InstructionContext { interpreter, host }),
        PUSH7 => stack_instructions::push::<7, _, _>(InstructionContext { interpreter, host }),
        PUSH8 => stack_instructions::push::<8, _, _>(InstructionContext { interpreter, host }),
        PUSH9 => stack_instructions::push::<9, _, _>(InstructionContext { interpreter, host }),
        PUSH10 => stack_instructions::push::<10, _, _>(InstructionContext { interpreter, host }),
        PUSH11 => stack_instructions::push::<11, _, _>(InstructionContext { interpreter, host }),
        PUSH12 => stack_instructions::push::<12, _, _>(InstructionContext { interpreter, host }),
        PUSH13 => stack_instructions::push::<13, _, _>(InstructionContext { interpreter, host }),
        PUSH14 => stack_instructions::push::<14, _, _>(InstructionContext { interpreter, host }),
        PUSH15 => stack_instructions::push::<15, _, _>(InstructionContext { interpreter, host }),
        PUSH16 => stack_instructions::push::<16, _, _>(InstructionContext { interpreter, host }),
        PUSH17 => stack_instructions::push::<17, _, _>(InstructionContext { interpreter, host }),
        PUSH18 => stack_instructions::push::<18, _, _>(InstructionContext { interpreter, host }),
        PUSH19 => stack_instructions::push::<19, _, _>(InstructionContext { interpreter, host }),
        PUSH20 => stack_instructions::push::<20, _, _>(InstructionContext { interpreter, host }),
        PUSH21 => stack_instructions::push::<21, _, _>(InstructionContext { interpreter, host }),
        PUSH22 => stack_instructions::push::<22, _, _>(InstructionContext { interpreter, host }),
        PUSH23 => stack_instructions::push::<23, _, _>(InstructionContext { interpreter, host }),
        PUSH24 => stack_instructions::push::<24, _, _>(InstructionContext { interpreter, host }),
        PUSH25 => stack_instructions::push::<25, _, _>(InstructionContext { interpreter, host }),
        PUSH26 => stack_instructions::push::<26, _, _>(InstructionContext { interpreter, host }),
        PUSH27 => stack_instructions::push::<27, _, _>(InstructionContext { interpreter, host }),
        PUSH28 => stack_instructions::push::<28, _, _>(InstructionContext { interpreter, host }),
        PUSH29 => stack_instructions::push::<29, _, _>(InstructionContext { interpreter, host }),
        PUSH30 => stack_instructions::push::<30, _, _>(InstructionContext { interpreter, host }),
        PUSH31 => stack_instructions::push::<31, _, _>(InstructionContext { interpreter, host }),
        PUSH32 => stack_instructions::push::<32, _, _>(InstructionContext { interpreter, host }),

        DUP1 => stack_instructions::dup::<1, _, _>(InstructionContext { interpreter, host }),
        DUP2 => stack_instructions::dup::<2, _, _>(InstructionContext { interpreter, host }),
        DUP3 => stack_instructions::dup::<3, _, _>(InstructionContext { interpreter, host }),
        DUP4 => stack_instructions::dup::<4, _, _>(InstructionContext { interpreter, host }),
        DUP5 => stack_instructions::dup::<5, _, _>(InstructionContext { interpreter, host }),
        DUP6 => stack_instructions::dup::<6, _, _>(InstructionContext { interpreter, host }),
        DUP7 => stack_instructions::dup::<7, _, _>(InstructionContext { interpreter, host }),
        DUP8 => stack_instructions::dup::<8, _, _>(InstructionContext { interpreter, host }),
        DUP9 => stack_instructions::dup::<9, _, _>(InstructionContext { interpreter, host }),
        DUP10 => stack_instructions::dup::<10, _, _>(InstructionContext { interpreter, host }),
        DUP11 => stack_instructions::dup::<11, _, _>(InstructionContext { interpreter, host }),
        DUP12 => stack_instructions::dup::<12, _, _>(InstructionContext { interpreter, host }),
        DUP13 => stack_instructions::dup::<13, _, _>(InstructionContext { interpreter, host }),
        DUP14 => stack_instructions::dup::<14, _, _>(InstructionContext { interpreter, host }),
        DUP15 => stack_instructions::dup::<15, _, _>(InstructionContext { interpreter, host }),
        DUP16 => stack_instructions::dup::<16, _, _>(InstructionContext { interpreter, host }),

        SWAP1 => stack_instructions::swap::<1, _, _>(InstructionContext { interpreter, host }),
        SWAP2 => stack_instructions::swap::<2, _, _>(InstructionContext { interpreter, host }),
        SWAP3 => stack_instructions::swap::<3, _, _>(InstructionContext { interpreter, host }),
        SWAP4 => stack_instructions::swap::<4, _, _>(InstructionContext { interpreter, host }),
        SWAP5 => stack_instructions::swap::<5, _, _>(InstructionContext { interpreter, host }),
        SWAP6 => stack_instructions::swap::<6, _, _>(InstructionContext { interpreter, host }),
        SWAP7 => stack_instructions::swap::<7, _, _>(InstructionContext { interpreter, host }),
        SWAP8 => stack_instructions::swap::<8, _, _>(InstructionContext { interpreter, host }),
        SWAP9 => stack_instructions::swap::<9, _, _>(InstructionContext { interpreter, host }),
        SWAP10 => stack_instructions::swap::<10, _, _>(InstructionContext { interpreter, host }),
        SWAP11 => stack_instructions::swap::<11, _, _>(InstructionContext { interpreter, host }),
        SWAP12 => stack_instructions::swap::<12, _, _>(InstructionContext { interpreter, host }),
        SWAP13 => stack_instructions::swap::<13, _, _>(InstructionContext { interpreter, host }),
        SWAP14 => stack_instructions::swap::<14, _, _>(InstructionContext { interpreter, host }),
        SWAP15 => stack_instructions::swap::<15, _, _>(InstructionContext { interpreter, host }),
        SWAP16 => stack_instructions::swap::<16, _, _>(InstructionContext { interpreter, host }),

        DUPN => stack_instructions::dupn(InstructionContext { interpreter, host }),
        SWAPN => stack_instructions::swapn(InstructionContext { interpreter, host }),
        EXCHANGE => stack_instructions::exchange(InstructionContext { interpreter, host }),

        LOG0 => host_instructions::log::<0, _>(InstructionContext { interpreter, host }),
        LOG1 => host_instructions::log::<1, _>(InstructionContext { interpreter, host }),
        LOG2 => host_instructions::log::<2, _>(InstructionContext { interpreter, host }),
        LOG3 => host_instructions::log::<3, _>(InstructionContext { interpreter, host }),
        LOG4 => host_instructions::log::<4, _>(InstructionContext { interpreter, host }),
        ...
        SELFDESTRUCT => host_instructions::selfdestruct(InstructionContext { interpreter, host }),
