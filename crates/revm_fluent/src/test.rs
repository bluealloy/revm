...
struct TxBuilder<'a> {
    pub(crate) ctx: &'a mut TestingContext,
    pub(crate) env: Env,
}
...
impl<'a> TxBuilder<'a> {
    ...
    fn exec(&mut self) -> Result<ExecutionResult, EVMError<Infallible>> {
...
fn deploy_evm_tx(ctx: &mut TestingContext, deployer: Address, init_bytecode: Bytes) -> Address {
    ...
    let result = TxBuilder::create(ctx, deployer, init_bytecode.clone().into())
        .exec()
        .unwrap();
...
fn test_deploy_greeting() {
    ...
    let contract_address = deploy_evm_tx(
        &mut ctx,
        DEPLOYER_ADDRESS,
        include_bytes!("../../../../fluentbase/examples/bin/greeting.wasm").into(),
    );
...
fn test_deploy_keccak256() {
    ...
    let contract_address = deploy_evm_tx(
        &mut ctx,
        DEPLOYER_ADDRESS,
        include_bytes!("../../../../fluentbase/examples/bin/keccak256.wasm").into(),
    );
    // call greeting WASM contract
    let result = call_evm_tx(
        &mut ctx,
...
fn test_deploy_panic() {
    ...
    let contract_address = deploy_evm_tx(
        &mut ctx,
        DEPLOYER_ADDRESS,
        include_bytes!("../../../../fluentbase/examples/bin/panic.wasm").into(),
    );
    // call greeting WASM contract
    let result = call_evm_tx(
        &mut ctx,
...
fn test_bridge_contract() {
    ...
    let exec_result = tx_builder.exec().unwrap();
    assert!(tx_builder.ctx.db.accounts.contains_key(&contract_address));
...
fn test_bridge_contract_with_call() {
    ...
    let result = pegged_token_factory_tx_builder.exec().unwrap();
    match result {
    ...
    let result = erc20token_factory_tx_builder.exec().unwrap();
    match result {
        ExecutionResult::Success { output, .. } => match output {
            Output::Create(_, address) => {
    ...
    let result = erc20gateway_factory_tx_builder.exec().unwrap();
    match result {
        ExecutionResult::Success { output, .. } => match output {
            Output::Create(_, address) => {
    ...
    let result = transfer_ownership_tx_builder.exec().unwrap();
    match result {
        ExecutionResult::Success { output, .. } => match output {
            Output::Call(bytes) => {
    ...
    let result = l1token_factory_tx_builder.exec().unwrap();
    match result {
        ExecutionResult::Success { output, .. } => match output {
            Output::Create(_, address) => {
...
lazy_static! {
    static ref EVM_LOADER: (Bytes, B256) = {
        let rwasm_bytecode: Bytes =
            include_bytes!("../../../../fluentbase/crates/contracts/assets/evm_loader_contract.rwasm").into();
